...
const textExpanded = ref(false) // 描述是否展开
...
// 提交分析任务
const submitAnalysis = async (imageUrls: string[], type: 'palm', text: string, gender: number, age: number) => {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 2. 提交分析任务
    const taskNo = await submitAnalysis(uploadedUrls, 'palm', aiText.value, gender.value, age.value)
  ...
  <div class="palm-page" v-loading="loading">
    ...
    <div class="upload-section">
      <div class="upload-tip">
        <span class="required-tag">必填</span> 拍摄清晰的手掌照片，至少上传一张
      </div>
      ...
      >
        <div class="upload-trigger">
          ...
          <span class="upload-text">{{ imageList.length > 0 ? '继续添加照片' : '点击上传手掌照片' }}</span>
          <span class="upload-hint">支持多张上传，建议拍摄掌心、掌背</span>
        </div>
      </el-upload>
    </div>
    ...
    <div class="ai-text-section">
      ...
      <div v-show="textExpanded" class="ai-text-content">
        <el-input
          ...
          type="textarea"
          :rows="4"
          placeholder="请描述您关注的手相特征（如：生命线长短、事业线清晰度、婚姻线情况等），AI将根据您的描述进行分析..."
          resize="none"
          maxlength="500"
          show-word-limit
        />
      ...
      >
        <template #default>
          <div class="disclaimer-content">
            <p>1. 本分析结果仅供参考，不能作为医疗诊断或命运预测依据。</p>
            <p>2. 手相学属于传统文化范畴，不具备科学验证依据。</p>
            <p>3. 请勿过分依赖手相分析结果，命运掌握在自己手中。</p>
          </div>
        </template>
      </el-alert>
    </div>
